...
types protos="protos":
    ...
    protos="$(find {{ protos }} -iname "*.proto" | xargs)" && \
        protoc --proto_path={{ protos }} \
            ...
            --prost_opt=type_attribute=hank.message.Reaction='#[cfg_attr(feature = "builder"\, derive(derive_builder::Builder))]' \
            --prost_opt=type_attribute=hank.message.Reaction='#[cfg_attr(feature = "builder"\, builder(default\, setter(into\, strip_option)\, custom_constructor\, build_fn(name = "fallible_build")))]' \
            --prost-crate_out=. \
    ...
    cat hank.message.customizations.rs >> src/hank.message.rs
